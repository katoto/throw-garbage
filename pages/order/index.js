...
const msgTempId = [
  ...
  "ovtzX9vUcTo4o0hPKRowULLuSSVww3e4YAhOtdT_GC4",
  "nQGN2OaiDuKnK_C73_VOxJkGcRQSexwIiF1gIz8xLn4",
];
...
Page(
  mixin(myBehavior, {
    ...
    data: {
      ...
      userAvatar: {},
      homeServer: utils.cache("banner")
        ? utils.cache("banner").homeServer
        : adConfig.banner.homeServer,
    },
    /**
     * 生命周期函数--监听页面加载
     */
    onLoad: function (options) {
      //不存在类型的时候跳转到选择页面
      app.handleLogin().then(async () => {
        this.checkUserAvatar();
      });
      this.getBannerList();
    },
    async checkUserAvatar() {
      // 处理头像不响应
      const isLog = this.checkUserLogin();
      if (isLog) {
        await com_getUserAvatar();
      }
      this.setData({
      ...
      wx.requestSubscribeMessage({
        ...
        success: (res) => {
          if (res["Hiir7mE7qPhb7Zk029NyoWAxDqr2IDK0O93rG_Zycb4"] === "accept") {
            ...
            res["UxNdeL8j7hUnosjh-2bfCI1aGuaayjzsRJJZ4nMv6Vc"] === "accept"
          ) {
