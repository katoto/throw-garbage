...
Page(mixin(myBehavior, {
    ...
    async coverHandler(res) {
        ...
        console.log('===nextFlagP1==', nextFlag)
        console.log('===是否是万能芯片p1==', colorfulListArr.includes(_nfcId))
        // 万能芯片和跳过
        if (!nextFlag && (!collectArr.includes(_nfcId) || colorfulListArr.includes(_nfcId))) {
            ...
            setTimeout(() => {
                // 并跳到下一个页面
                nfcAdapter = null;
                openPageByType('mini://pages/act/place/place2?oid=' + _that.data.oid, {
                    linkType: 'redirect'
                })
            }, 1000)
    ...
    isCurrentPage() {
        ...
        return url === 'pages/act/place/place'
    },
    checkPlaceFn(nfcId) {
    ...
    goTestFn() {
        if (env_version === 'develop') {
            cache(cacheKey, 'testid1')
            openPageByType('mini://pages/act/place/place2?oid=' + this.data.oid, {
                linkType: 'redirect'
            })
        }
    },
